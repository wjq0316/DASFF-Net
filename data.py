...
class SalObjDataset(data.Dataset):
    ...
    def filter_files(self):
        ...
        # 修改断言条件，检查三个列表的长度是否一致
        assert len(self.images) == len(self.gts) and len(self.gts) == len(self.depths), \
            f"Number of images, gts, and depths must match. Got: images={len(self.images)}, gts={len(self.gts)}, depths={len(self.depths)}"
        images = []
        gts = []
        depths = []
...
class test_dataset:
    def __init__(self, image_root, gt_root, depth_root, testsize):
        ...
        self.transform = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize(mean=(0.485, 0.458, 0.407), std=(0.229, 0.224, 0.225))])
...
if __name__ == "__main__":
    ir = "E:/lmt/train_dut/train_dut/train_images/"
    dr = "E:/lmt/train_dut/train_dut/train_depth/"
    gt = "E:/lmt/train_dut/train_dut/train_masks/"
    b = 8
    ts = 384
    train_loader = get_loader(ir, gt, dr, b, ts)
